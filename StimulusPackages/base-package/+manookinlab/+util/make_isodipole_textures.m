function S = make_isodipole_textures(xsize,ysize,tsize,glider,parity,seed)
...
rows=64; cols=64;
fprop=1; %input('propagated decorrelation fraction (0=even, 1=odd, 0.5=random):');
fspor=0; %input('sporadic decorrelation fraction (0=even, 0.5=random):');
tex2=repmat((rand(1,cols)>0.5),rows,1)+repmat(rand(rows,1)>0.5,1,cols); %even texture
tex2=mod(tex2+cumsum(cumsum((rand(rows,cols)<fprop)),2)+(rand(rows,cols)<fspor),2); %add decorrelation
imagesc(tex2);

seed=1;
obj.noiseStream = RandStream('mt19937ar', 'Seed', seed);
obj.numYStixels=64; obj.numXStixels=64;
parity=0;
obj.seed=1;
imagesc(squeeze(makeGlider(obj.numYStixels,1, obj.numXStixels, [0 1 1; 1 1 0], parity, obj.seed+1)))
imagesc(squeeze(makeGlider(obj.numYStixels,1, obj.numXStixels, [0 0 0;0 1 0;1 0 1], parity, obj.seed+1)))
imagesc(squeeze(makeGlider(obj.numYStixels,1, obj.numXStixels, [0 0 0;1 0 0;1 0 1;2 0 1], parity, obj.seed+1)))
imagesc(squeeze(makeGlider(obj.numYStixels,1, obj.numXStixels, [0 0 0;0 0 1;1 0 1;1 0 2], parity, obj.seed+1)))
imagesc(squeeze(makeGlider(obj.numYStixels,1, obj.numXStixels, [0 0 0;1 0 0;0 1 0;1 0 1], parity, obj.seed+1)))
imagesc(squeeze(makeGlider(obj.numYStixels,1, obj.numXStixels, [0 0 0;1 0 0;1 0 1], parity, obj.seed+1)))
