classdef LinearSumDtBars < manookinlab.protocols.ManookinLabStageProtocol
    properties
        ...
        dtVals = [0:2,4:2:8]            % Temporal offset of paired flashes (frames)
...
%             end
            if ~strcmp(obj.onlineAnalysis, 'none')
                obj.showFigure('manookinlab.figures.LinearSumFigure', ...
                    ...
                    'sortName', 'dx', 'sortValues', unique(obj.dtVals));
            end
            
            % dtSeq
            numReps = ceil(double(obj.numberOfAverages)/(3*length(obj.dtVals)));
            obj.dtSeq = ones(3,1) * obj.dtVals(:)';
            obj.dtSeq = obj.dtSeq(:) * ones(1, numReps);
            obj.dtSeq = obj.dtSeq(:)';
            obj.dtSeq = obj.dtSeq( 1 : obj.numberOfAverages );
        ...
        function prepareEpoch(obj, epoch)
            ...
            obj.dtFrames = obj.dtSeq( obj.numEpochsCompleted+1 );
            % Calculate dt in seconds.
            obj.dtSec = obj.dtFrames/obj.frameRate;

            % Save the frame delay
            epoch.addParameter('stimulusName', obj.stimulusName);
            epoch.addParameter('pulseTime', obj.pulseTime);
            epoch.addParameter('dtFrames', obj.dtFrames);
            epoch.addParameter('dtMsec', obj.dtSec*1e3);
        end
