classdef AdaptNoiseLuminanceSteps < manookinlab.protocols.ManookinLabStageProtocol
    properties
        ...
        monitorMeans = [0.5,0.05,0.05,0.02]
        gainMeans = [1,1,0.1,0.025]
        contrasts = [1,1]
                ...
                if frame > 0
                    if mod(frame, obj.frameDwell) == 0
                        ...
                        if frame <= obj.unique_frames
                            M = obj.contrast*(2* (obj.noiseStream.rand(obj.numYStixels,obj.numXStixels)>0.5)-1);
                        else
                            M = obj.contrast*(2*(obj.noiseStream.rand(obj.numYStixels,obj.numXStixels)>0.5)-1);
                        end
            ...
            function s = setRGBStixels(obj, frame)
                ...
                if frame > 0
                    if mod(frame, obj.frameDwell) == 0
                        m_idx = min(floor(frame/obj.step_frames) + 1, length(obj.mean_steps));
                        if frame <= obj.unique_frames
                            M = obj.contrast*(2*(obj.noiseStream.rand(obj.numYStixels,obj.numXStixels,3)>0.5)-1);
                        else
                            M = obj.contrast*(2*(obj.noiseStream.rand(obj.numYStixels,obj.numXStixels,3)>0.5)-1);
                        end
                        M = M*obj.mean_steps(m_idx) + obj.mean_steps(m_idx);
