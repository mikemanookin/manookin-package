classdef NaturalImageAndMovingBar < edu.washington.riekelab.manookin.protocols.ManookinLabStageProtocol
    properties
        amp                             % Output amplifier
        preTime = 250                   % Stimulus leading duration (ms)
        stimTime = 5500                 % Stimulus duration (ms)
        tailTime = 250                  % Stimulus trailing duration (ms)
        waitTime = 4000                 % Stimulus wait duration (ms)
        barSize = [250 250]             % Bar size [w, h] (pix).
        barContrast = 0.3               % Bar contrast (-1:1)
        barSpeed = 2000                 % Bar speed (pix/sec)
        barOrientation = 0.0            % Bar orientation in degrees
        motionSD = 25                   % Motion standard dev (pixels)
        stimulusIndex = 2               % Stimulus number (1:161)
        maskDiameter = 300                % Mask diameter in pixels
    ...
    methods
        ...
        function p = createPresentation(obj)
            ...
            function p = motionTable(obj, time)
                ...
                inc = time * obj.barSpeed - 570 - obj.barSize(1)/2 ;
